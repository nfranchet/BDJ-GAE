...
function getListeAlbums(lettre) {
	...
	$.get("bdj_gae?action=series&lettre="+lettre.charCodeAt(0), function(data) {
...
function afficheSerie(idBel) {
	$.get("bdj_gae?action=albumResume&idBel="+idBel, function(data) {
		// On alimente le div response
		if (data.substring(0,"[ERROR]".length) == "[ERROR]") {
			alert(data.replace(["ERROR"],"","g")); // On affiche le message d'erreur dans une boite d'alerte
		} else {
...
function afficheParaBD(idBel) {
	$.get("bdj_gae?action=parabdResume&idBel="+idBel, function(data) {
		// On alimente le div response
		if (data.substring(0,"[ERROR]".length) == "[ERROR]") {
			alert(data.replace(["ERROR"],"","g")); // On affiche le message d'erreur dans une boite d'alerte
		} else {
...
function afficheRevues(nom) {
	$.get("bdj_gae?action=revuesResume&nom="+nom, function(data) {
		// On alimente le div response
		if (data.substring(0,"[ERROR]".length) == "[ERROR]") {
			alert(data.replace(["ERROR"],"","g")); // On affiche le message d'erreur dans une boite d'alerte
		} else {
...
function getRevues(lettre) {
	...
	$("#listeResume").html("");
	// TODO : Pour l'instant on ne gere pas la langue, amélioration a faire 
	$.get("bdj_gae?action=revues&lettre="+lettre.charCodeAt(0), function(data) {
		populateListeObject(data);
	});
}
...
function getParaBD(lettre) {
	...
	$("#listeResume").html("");
	// TODO : Pour l'instant on ne gere pas la langue, amélioration a faire 
	$.get("bdj_gae?action=paradBDs&lettre="+lettre.charCodeAt(0), function(data) {
		populateListeObject(data);
	});
}